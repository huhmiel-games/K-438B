...
export default class Player extends Phaser.GameObjects.Sprite{
  constructor(scene, x, y, config) {
    ...
    const { LEFT, RIGHT, UP, DOWN, SPACE, SHIFT, ENTER, P,D }  = Phaser.Input.Keyboard.KeyCodes;
    ...
    this.ComboMorphingBall = this.scene.input.keyboard.createCombo([this.keys.down, this.keys.down], {
      resetOnWrongKey: true,
      maxKeyDelay: 500,
      resetOnMatch: true,
      deleteOnMatch: false,
    });
    this.ComboJumpBooster = this.scene.input.keyboard.createCombo([this.keys.down, this.keys.jump], {
      resetOnWrongKey: true,
      maxKeyDelay: 500,
      resetOnMatch: true,
      deleteOnMatch: false,
    });
    this.scene.input.keyboard.on('keycombomatch', function (keyCombo, keyboardEvent) { 
  ...
  preUpdate (time, delta) {
    ...
    // if ( keys.pause.isDown && !this.state.pause ) {
    //   this.scene.pauseGame(true)
    // } else if ( keys.pause.isDown && this.state.pause ) {
    //   this.scene.pauseGame(false)
    // }
    ...
    if ( !this.state.pause ) {
      ...
      } else if ( !this.body.blocked.down && !( keys.left.isDown || keys.right.isDown ) && !this.state.onMorphingBall  && !this.state.jumpBoost ) {
          animationName = 'jumpVertical';
          this.body.setVelocityX(0);
          this.ComboJumpBooster.enabled = false;
        // saut avec booster
      } else if ( this.state.jumpBoost && !( keys.left.isDown || keys.right.isDown ) && !this.state.onMorphingBall ) {
  ...
  isRunning () {
    if ( this.keys.run.isDown && ( this.keys.left.isDown || this.keys.right.isDown ) && this.body.blocked.down ) {
  ...
  shoot (time) {
    if ( time > this.state.lastFired && this.inventory.gun){
      let bullet = this.bullets.getFirstDead(true, this.body.x + this.state.bulletPositionX, this.body.y + this.state.bulletPositionY, 'bullet', null, true);
      if ( bullet ) {
